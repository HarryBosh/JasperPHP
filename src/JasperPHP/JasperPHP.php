...
class JasperPHP
{
    ...
    public function process($resource= false, $input_file, $output_file = false, $format = array("pdf"), $parameters = array(), $db_connection = array(), $background = true, $redirect_output = true)
    {
        dump("printing...");
        if(is_null($input_file) || empty($input_file))
            throw new \Exception("No input file", 1);
    ...
    public function execute($run_as_user = false)
    {
        ...
        if($return_var != 0) {
            ...
            \Log::error('Error printing', [
                'command' => $this->the_command,
                'output'  => $output
                ]);

            
            dump("There was a problem printing, developer has been notified.");
            throw new \Exception("error exec() check apache error.log", 1);
        }
